...
namespace Prex.Utils
{
    public static class ManejarErrores
    {
        ...
        public static void TratarError(Exception ex, string funcion) => TratarError(ex, funcion, string.Empty, true);
        public static void TratarError(Exception ex, string funcion, string customError, bool guardaLog)
        {
            var frm = new frmError();
            frm.CargarFormulario(ex, funcion, customError);
            if (guardaLog)
