Module modGlobalFunciones
    ...
    Public Sub LeerINI()
        ...
        Try
            ...
            EDICION_SQL_HABILITADA = INIRead(sIniPath, "OPCIONES", "SQL_EDIT_ENABLED", "1")
            AQB_MOSTRAR_TABLAS = INIRead(sIniPath, "OPCIONES", "SHOW_TABLES", "1")
            AQB_MOSTRAR_VISTAS = INIRead(sIniPath, "OPCIONES", "SHOW_VIEWS", "1")
            AQB_MOSTRAR_SPS = INIRead(sIniPath, "OPCIONES", "SHOW_PROCEDURES", "1")
            ...
            sTemp = INIRead(sIniPath, "OPCIONES", "ENABLE_CMD_LOGIN", "0")

            If IsNumeric(sTemp) Then
                If Val(sTemp) = 1 Then
                    CMDLINE_HABILITAR_LOGIN = True
                End If
            End If

            sTemp = INIRead(sIniPath, "OPCIONES", "OBJECT_IDENTIFIER", "[]")

            If sTemp.Length > 1 Then
                OBJECT_IDENTIFIER_START = sTemp.Substring(0, 1)
                OBJECT_IDENTIFIER_CLOSE = sTemp.Substring(1, 1)
            Else
                OBJECT_IDENTIFIER_START = "["
                OBJECT_IDENTIFIER_CLOSE = "]"
            End If
    ...
    Public Sub LeerINIUsuario()
        ...
        Try
            ...
            If Not FileExists(sLocalIniPath) Then
                ...
                oWriter.WriteLine("COLLAPSE_RIBBON=1")
                oWriter.WriteLine("THEME_ACTUAL=Office 2007 Blue")
                oWriter.WriteLine("DESIGN_SPLIT=1")
                oWriter.WriteLine("USE_INTELLIPROMPT=0")
                oWriter.WriteLine("SHOW_WELCOME=1")
                oWriter.WriteLine("MULTIFILTER_COLUMNS=0")
                oWriter.WriteLine("SHOW_PERCENT_PB=1")
                oWriter.Flush()
            End If

            IDIOMA_ACTUAL = INIRead(sLocalIniPath, "OPCIONES", "IDIOMA", "1")
            AUTOSAVE_LAYOUTS = INIRead(sLocalIniPath, "OPCIONES", "AUTOSAVE_LAYOUTS", "0")
            COLLAPSE_RIBBON = INIRead(sLocalIniPath, "OPCIONES", "COLLAPSE_RIBBON", "1")
            THEME_ACTUAL = INIRead(sLocalIniPath, "OPCIONES", "THEME_ACTUAL", "Office 2007 Blue")
            DESIGN_SPLIT = INIRead(sLocalIniPath, "OPCIONES", "DESIGN_SPLIT", "1")
            USE_INTELLIPROMPT = INIRead(sLocalIniPath, "OPCIONES", "USE_INTELLIPROMPT", "0")
            SHOW_WELCOME = INIRead(sLocalIniPath, "OPCIONES", "SHOW_WELCOME", "1")
            MULTIFILTER_COLUMNS = INIRead(sLocalIniPath, "OPCIONES", "MULTIFILTER_COLUMNS", "0")
            SHOW_PERCENT_PB = INIRead(sLocalIniPath, "OPCIONES", "SHOW_PERCENT_PB", "0")
    ...
    Public Function AbrirConsultaGlobal(ByVal nCodigoConsulta As Integer) As Consulta
        ...
        Try
            ...
            With oUDT
                ...
                .ProtegidaDiseno = (dsAux.Tables(0).Rows(0)("CV_PASSDS") = 1)
                .PasswordDiseno = dsAux.Tables(0).Rows(0)("CV_PASSDG")
                ...
                For nI = 0 To dsAux.Tables(1).Rows.Count - 1
                    ...
                    .Detalles(nI).Valor = ""
                    .Detalles(nI).UsaBusquedaPrevia = (dsAux.Tables(1).Rows(nI)("CD_USABUS") = 1)
                    .Detalles(nI).MensajeBusqueda = dsAux.Tables(1).Rows(nI)("CD_MSJBUS")
                    .Detalles(nI).VariableBusqueda = dsAux.Tables(1).Rows(nI)("CD_VARBUS")
                Next
                ...
                For nI = 0 To dsAux.Tables(2).Rows.Count - 1
                    ...
                    .Formatos(nI).Titulo = dsAux.Tables(2).Rows(nI)("CF_TITULO")
                    .Formatos(nI).Visible = dsAux.Tables(2).Rows(nI)("CF_VISIBL")
                Next
