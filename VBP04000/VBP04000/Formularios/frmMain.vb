...
Public Class frmMain
			...
			"WHERE     MU_CODTRA = " & nCodigoTransaccion
				...
				With ds.Tables(0)
					...
					Else
						...
						Me.Text = "Transacción:" & CODIGO_TRANSACCION.ToString & " - " & .Rows(0).Item("MU_TRANSA").ToString
			...
			For Each detalle As DetalleConsulta In udtConsultaActual.Detalles.OrderBy(Function(d) d.Orden)
				...
				Select Case detalle.Help
					...
					Case 1
						...
						AddHandler oLookUp.CustomDisplayText, AddressOf Fecha_CustomDisplayText
					Case 2
						oLookUp = New RepositoryItemTextEdit()
						oLookUp.Name = "input" & detalle.Variable
				End Select
	...
	Private Sub Fecha_CustomDisplayText(sender As Object, e As CustomDisplayTextEventArgs)
		If e.Value Is Nothing OrElse e.Value.ToString.Trim = String.Empty Then Exit Sub
		Dim r As DateTime
		If DateTime.TryParse(e.Value, r) Then
			e.DisplayText = r.ToString("dd/MM/yyyy")
		End If
	End Sub
