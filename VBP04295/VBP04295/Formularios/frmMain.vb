...
Maneja_Error:
   ...
   Private Sub GenerarReporte()
      ...
      sSQL = "SELECT    * " & _
             "FROM      REPORT " & _
             "WHERE     RO_CODENT IN (@CODENT, 0) " & _
             "AND       RO_CODTRA = " & CODIGO_TRANSACCION & " " & _
             "AND       RO_FECVIG = ( " & _
             "                       SELECT   MAX(RO_FECVIG) " & _
             "                       FROM     REPORT " & _
             "                       WHERE    RO_CODENT IN (@CODENT, 0) " & _
             "                       AND      RO_CODTRA = " & CODIGO_TRANSACCION & " " & _
             "                       AND      RO_FECVIG <= @FECVIG) " & _
      ...
      If CODIGO_TRANSACCION = 14001 Then
         sSQL = "SELECT       DATCUA.*, TG_DESCRI " & _
                "FROM         DATCUA " & _
                "LEFT JOIN    TABGEN " & _
                "ON           DC_CODENT = TG_CODCON " & _
                "AND          TG_CODTAB = 1 " & _
                "WHERE        DC_CUADRO = @CUADRO " & _
                "AND          DC_FECVIG = @FECVIG " & _
                "AND          DC_CODENT = @CODENT " & _
                "AND          CAST(DC_CODCON AS INT) = @CODCON " & _
                "AND          DC_ACTIVA = 1 " & _
                "AND          DC_NIVELTAB = 1 " & _
            ...
            If (nPag >= nMax) Or (InStr(1, sFuente, "<SALTO_PAGINA/>", vbTextCompare)) Then
               ...
               For nPos1 = 1 To UBound(Detalle)
                  If UCase(Detalle(nPos1).Variable) = "SALTO_PAGINA" Then
                     If (CStr(nPagina + 1) = CStr(Detalle(nPos1).Condicion) And _
                         Val(Detalle(nPos1).Condicion) <> 0) Or _
...
Reinicio:
   ...
   Private Function SaltoPagina(ByRef sDetalle As String, _
                                ByRef oDoc As ABCpdf7.Doc, _
                                ByRef nPag As Long, _
                                ByRef nMax As Integer, _
                                ByRef bHorizontal As Boolean, _
                                ByRef sHTML As String, _
                                ByRef sEncPag As String, _
                                ByRef sPiePag As String, _
                                ByRef sSalida As String, _
                                ByRef sPag As String, _
                                ByRef bFlag As Boolean, _
                                ByRef bAdicionar As Boolean, _
